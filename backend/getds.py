...
def get_messages_rag( history):
    ...
    while retry <= 1:
        try:
            messages = [
                {'role': 'system',
                 'content': '你是一个语义分析专家，专门从用户的多轮对话中提取出与当前问题相关的关键词，以便用于搜索产品资料。关键词应尽可能具体准确。如果用户的问题与产品、健康等需要查资料的内容无关（如闲聊、问你是谁等），请返回关键词：无。'},
                {'role': 'user',
                 'content': f'以下是用户和助手的对话历史：\n\n{history}\n\n根据最新一轮用户的问题（最后用户user的话）判断是否需要提取关键词用于查询资料，如果不需要则返回<无>，需要的话判断最相关的关键词（如产品名、功效成分、相关话题），用于搜索数据库。\n\n仅返回格式如下：\n关键词：<关键词内容>\n\n如果没有相关关键词，请返回：\n关键词：无'}
            ]
            client = OpenAI(
