...
class Ws_Param(object):
    # 初始化
    def __init__(self, APPID, APIKey, APISecret, Text,File_path):
        self.APPID = APPID
        self.APIKey = APIKey
...
def on_open(ws):
    def run(*args):
        ...
        print("------>开始发送文本数据")
        ws.send(d)
        if os.path.exists(ws.path):#pcm
            os.remove(ws.path)

    thread.start_new_thread(run, ())

def vtw(word_str,save_folder='./uploads/tts/'):
    try:
        ...
        wsParam = Ws_Param(APPID='5f30a0b3', APISecret='MGYyMzJlYmYzZWVmMjIxZWE4ZThhNzA4',
                           ...
                           Text=word_str,File_path=file_path)
