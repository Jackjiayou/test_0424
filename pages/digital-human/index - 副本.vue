	...
	export default {
		...
		methods: {
			...
			async switchVideo(newUrl) {
				...
				// 预加载下一个视频
				const nextVideoElement = document.querySelector('.next-video');
				if (nextVideoElement) {
				...
				// 淡出当前视频，淡入下一个视频
				const duration = 300; // 减少过渡时间到300ms
				const steps = 30; // 增加步数使过渡更平滑
				const stepTime = duration / steps;
				
				// 使用 requestAnimationFrame 实现更平滑的过渡
